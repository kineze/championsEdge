  ...
  <div class="relative mx-auto max-w-3xl px-6">
    <div class="rounded-3xl border border-slate-200 bg-white p-8 shadow-xl dark:border-slate-700 dark:bg-slate-900">
      ...
      <div class="mt-6 grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm dark:border-slate-700 dark:bg-slate-950/60">
        <p><span class="font-semibold">Facility:</span> {{ $subscription?->facility?->title ?? '-' }}</p>
        <p><span class="font-semibold">Plan:</span> {{ ucfirst((string) ($subscription?->plan?->frequency ?? '-')) }} - LKR {{ number_format((float) ($subscription?->plan?->price ?? 0), 2) }}</p>
        <p><span class="font-semibold">New Start Date:</span> {{ optional($subscription?->subscription_start_date)->format('Y-m-d') }}</p>
        <p><span class="font-semibold">New End Date:</span> {{ optional($subscription?->subscription_end_date)->format('Y-m-d') }}</p>
        <p><span class="font-semibold">Payment Reference:</span> {{ $payment->transaction_id ?: $payment->order_gateway_id }}</p>
      </div>

      <div class="mt-7 flex flex-wrap gap-3">
        <a href="{{ route('memberDashboard') }}" class="rounded-xl bg-cyan-600 px-5 py-2.5 text-sm font-bold text-white transition hover:bg-cyan-700">Go to Member Dashboard</a>
